...
if [ ! -e "${FRAMEWORKS_REPOSITORY}/WebObjects/WOInstaller.jar" ]; then
    echo "WOInstaller.jar is not in the Frameworks Repository (${FRAMEWORKS_REPOSITORY}/WebObjects). It will be downloaded from https://dl.dropbox.com/u/1548210/Jenkins/WOInstaller.jar"
	cd ${FRAMEWORKS_REPOSITORY}/WebObjects/
	curl -O https://dl.dropbox.com/u/1548210/Jenkins/WOInstaller.jar
fi
...
if [ ! -e "${FRAMEWORKS_REPOSITORY}/WOProject/woproject.jar" ]; then
    echo "woproject.jar is not in the Frameworks Repository (${FRAMEWORKS_REPOSITORY}/WOProject). It will be downloaded from https://dl.dropbox.com/u/1548210/Jenkins/woproject.jar"
	cd ${FRAMEWORKS_REPOSITORY}/WOProject/
	curl -O https://dl.dropbox.com/u/1548210/Jenkins/woproject.jar
fi
